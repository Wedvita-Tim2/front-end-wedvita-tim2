...
const TemplatesRendering = () =>{
    ...
    const templateCard = templates.map((template, id) => (
        <div className="relative group overflow-hidden bg-slate-50 ring-offset-2 ring-1 ring-slate-100 rounded-xl h-60 z-20 w-36 md:w-72 md:h-[460px] shadow-lg mx-auto mt-5 hover:scale-110 delay-100 duration-200 ease-in-out" key={id}>
          ...
          <div className="absolute bottom-0 inset-x-0 px-3 py-2 md:py-4 text-center group-hover:backdrop-blur-sm" style={{ backdropFilter: 'blur(2px)' }}>
            <p className="text-xs font-semibold md:text-lg text-white mb-2 md:mb-3 ">{template.template_name}</p>
            <Button type="button" onClick={() => handleSelectTemplate(template.id, template.template_name, 'order')} className="hidden mx-auto text-white bg-light-pink/70 md:py-2 rounded-xl py-1 w-3/4 group-hover:block">
                Order Now
            </Button>
          </div>
