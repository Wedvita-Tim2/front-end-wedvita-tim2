...
const Navbar = () => {
    ...
    if (!isLogin) {
      return (
        <Button
          type={"link"}
          href={"/login"}
          className={`${isLoginPage || isRegisterPage ? "hidden" : ""} mx-auto`}
          isGradient
        >
          Login
        </Button>
      ...
      const username =
        auth.dataUser.username.slice(0, 1).toUpperCase() +
        auth.dataUser.username.slice(1);
      return (
        <>
          <Button
            className={`font-bold text-2xl mx-auto text-center ${
              activePage === "/myaccount"
                ? "text-light-pink"
                : "text-primary-300"
            } py-2 px-8 hover:underline`}
            type={"link"}
            href={"/myaccount"}
          >
            <div className="flex">
  ...
  return (
    <>
      <header
        className={`${isLoginPage || isRegisterPage ? "" : "bg-white"} ${
          !isShow ? "hidden" : ""
        } top-0 left-0 w-full flex items-center z-[99999] ${
          isNavbarFixed && !(isLoginPage || isRegisterPage)
            ? "fixed shadow-md"
            : ""
        }`}
      >
        <div className="flex item-center justify-between relative w-full ">
          ...
          <div className={`flex items-center`}>
            <Button
              className={`${
                isLoginPage || isRegisterPage ? "hidden" : "block"
              } absolute right-3 mr-9 md:hidden`}
              type={"button"}
              onClick={hamburgerClick}
            >
              <span
                className={`w-6 h-1 my-1 block bg-primary-300 duration-300 ease-in-out origin-top-left ${
                  isMenuOpen ? "rotate-45 " : ""
                }`}
              ></span>
              <span
                className={`w-6 h-1 my-1 block bg-primary-300 duration-300 ease-in-out ${
                  isMenuOpen ? "scale-0" : ""
                }`}
              ></span>
              <span
                className={`w-6 h-1 my-1 block bg-primary-300 duration-300 ease-in-out origin-bottom-left ${
                  isMenuOpen ? "-rotate-45 " : ""
                }`}
              ></span>
            </Button>
            <nav
              className={`${isMenuOpen ? "" : "hidden"} ${
                isLoginPage || isRegisterPage ? "hidden" : ""
